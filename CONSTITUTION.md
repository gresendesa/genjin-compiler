...
Criação de uma linguagem DSL e compilador para a notação Genjin.
## 2. Regras Nao Negociaveis

1. Contratos de API nao podem ser quebrados sem versionamento.
2. Toda migracao deve ser reversivel.
3. Migracoes de dados para a nova arquitetura sao obrigatorias quando houver refatoracao estrutural.
4. Nenhuma entrega e concluida sem teste manual documentado.
5. Nenhuma entrega e concluida sem checklist de regressao executado.
...
- Apenas o owner (gresendesa) aprova mudancas na constituicao e diretrizes de memoria.
...
A pasta ia/ e a memoria operacional do projeto e deve seguir estas regras:
...
2. backlog-items/B-XXX.md
- Cada item de backlog deve ter um arquivo proprio em ia/backlog-items/.
...
4. sprints/SPR-YYYY-NN.md
- Cada sprint deve ter um arquivo proprio em ia/sprints/.
- O arquivo detalhado deve conter planning, tarefas, risco, execucao, bloqueios e encerramento.

5. architecture.md
- Deve ser atualizado sempre que houver mudanca de contrato, componente ou fluxo de integracao.
- Deve manter secao de arquitetura atual e arquitetura alvo (futura).
...
Todo registro em ia/ deve incluir:
- status
- owner
- data de criacao
